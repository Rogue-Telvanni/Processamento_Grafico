...
int setupGeometry();
int parseObj(string filePath, int &numVertices);
...
int parseObj(string filePath, int &numVertices)
{
	...
	vector<GLfloat> vbuffer;// ordem do buffer de triangulos
	...
	if(file)
	{
		...
		while(!file.eof())
		{
			...
			{
				glm::vec3 vertice;
				stream >> vertice.x >> vertice.y >> vertice.z;
				cout << vertice.x << " " << vertice.y << " " << vertice.z;
